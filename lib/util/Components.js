...
function componentRule(rule, context) {
  ...
  const utils = {
     ...
     */
    isDestructuredFromPragmaImport(variable, node) {
      return isDestructuredFromPragmaImport(variable, node, context);
    },
    ...
    getParentComponent(node) {
      return (
        componentUtil.getParentES6Component(node, context)
        || componentUtil.getParentES5Component(node, context)
    ...
    getRelatedComponent(node) {
      ...
      let variableInScope;
      const variables = variableUtil.variablesInScope(node, context);
    ...
    isReactHookCall(node, expectedHookNames) {
      ...
      const scope = (isPotentialReactHookCall || isPotentialHookCall) && context.sourceCode.getScope(node);
