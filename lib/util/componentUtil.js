 ...
 */
function getParentES5Component(node, context) {
 ...
 * @returns {ASTNode | null}
 */
function getParentES6Component(node, context) {
  let scope = context.sourceCode.getScope(node);
  while (scope && scope.type !== 'class') {
    scope = scope.upper;
  }
  const scopeBlock = scope && scope.block;
  if (!scopeBlock || !isES6Component(scopeBlock, context)) {
    return null;
  }
  return scopeBlock;
}
