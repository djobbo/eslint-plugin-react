 ...
 */
function inLifeCycleMethod(node, context, checkAsyncSafeLifeCycles) {
...
function isPropArgumentInSetStateUpdater(node, context, name) {
 ...
 * @returns {boolean}
 */
function isInClassComponent(node, context) {
  return !!(componentUtil.getParentES6Component(node, context) || componentUtil.getParentES5Component(node, context));
}
...
function isPropTypesUsageByMemberExpression(node, context, utils, checkAsyncSafeLifeCycles) {
  ...
  if (isInClassComponent(node, context)) {
    // this.props.*
    ...
    if (
      ...
      && (inLifeCycleMethod(node, context, checkAsyncSafeLifeCycles) || astUtil.inConstructor(node, context))
    ) {
      return true;
    }
    // this.setState((_, props) => props.*))
    if (isPropArgumentInSetStateUpdater(node, context, unwrappedObjectNode.name)) {
      return true;
    }
    return false;
  }
...
module.exports = function usedPropTypesInstructions(context, components, utils) {
  ...
  return {
    VariableDeclarator(node) {
      ...
      if (isThisDotProps(unwrappedInitNode) && isInClassComponent(node, context) && node.id.type === 'Identifier') {
      ...
      if (isThisDotProps(unwrappedInitNode) && isInClassComponent(node, context)) {
        markPropTypesAsUsed(node.id);
        return;
      }
