...
module.exports = {
  ...
  create(context) {
    function findSpreadVariable(node, name) {
      return variableUtil.variablesInScope(node, context).find((item) => item.name === name);
    }
    /**
      ...
      CallExpression(node) {
        if (
          ...
          if (props.type === 'Identifier') {
            const variable = variableUtil.variablesInScope(node, context).find((item) => item.name === props.name);
            if (variable && variable.defs.length && variable.defs[0].node.init) {
